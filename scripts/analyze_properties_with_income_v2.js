...
async function analyzeCommercialLand() {
  ...
  let maxIncomesFound = 0; // NEW: Track the maximum number of incomes found for any land property
  ...
  for (const landPropertyFeature of commercialLandFeatures) {
    ...
    // NEW: Update maxIncomesFound for dynamic columns
    ...
    const baseResult = {
      ...
      _nearbySalaries: nearbySalaries, // NEW: Store raw array for CSV construction
    };
  ...
  if (results.length > 0) {
    ...
    const outputCsvRows = results.map((row) => {
      ...
      for (let i = 0; i < maxCompetitorsFound; i++) {
        ...
        rowValues.push(
          competitor.name ? `COMPETITOR ${i + 1}` : "", // Re-adding COMPETITOR X header value as per previous version
          competitor.category || "",
          competitor.name || "",
          competitor.url || ""
        );
