 ...
 * where the lowest value scores a maximum (e.g., 20) and the highest scores a minimum (e.g., 0).
 * This is used for "low is good" metrics.
 ...
 */
function calculateInverseScore(value, min, max, maxScore = 20, minScore = 0) {
  if (isNaN(value)) {
    return minScore; // Default to minimum score for unparseable values
  }
  if (min === max) {
    return maxScore; // If all values are the same, the "best" score is assigned
  }

  // Linear interpolation: Score = MaxScore - ((Value - MinValue) / (MaxValue - MinValue)) * (MaxScore - MinScore)
  let score = maxScore - ((value - min) / (max - min)) * (maxScore - minScore);
  ...
  try {
    ...
    const intermediateData = records.map((row) => {
      ...
      processedRow["Income Average"] =
        incomeCount > 0 ? incomeSum / incomeCount : 0.0;
    ...
    console.log(
      "[INFO] Calculating Price Score, Price Rank, Competition Score, and Competition Rank..."
    );
    intermediateData.forEach((row) => {
      // Price Score
      row["Price Score"] = calculateInverseScore(
        row["£ per 1000 Sq Ft"],
        minMax["pricePer1000SqFt"].min,
        minMax["pricePer1000SqFt"].max
      );
      // Competition Score
      row["Competition Score"] = calculateInverseScore(
    ...
    // Define the exact output headers and their order, ensuring new columns are in place
    const outputHeaders = [
      "Size (acres)",
      "Town",
      "Rightmove url",
      ...
      "Price Rank",
      "Competition Total",
      "Competition Score", // Added right after Competition Total
      "Competition Rank", // Added right after Competition Score
      "Population Total",
      "Income Average",
      "Traffic Total",
    ];
    ...
    const finalRecords = intermediateData.map((row) => {
      ...
      outputHeaders.forEach((col) => {
        ...
        if (
          ...
          value =
            typeof value === "number" && !isNaN(value)
              ? value.toFixed(2)
              : "0.00";
        } else if (col === "Price Score" || col === "Competition Score") {
          value =
            typeof value === "number" && !isNaN(value)
              ? value.toFixed(2)
              : "0.00";
        ...
        else if (col === "Price Rank" || col === "Competition Rank") {
          value =
