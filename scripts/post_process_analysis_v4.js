...
function calculateScore(
  ...
  if (highIsGood) {
    ...
    // Inverse proportionality: (max - value) / (max - min) maps min to 0, max to 1
    // Then scale to (maxScore - minScore) range and shift by minScore
...
function assignRanks(data, scoreColumn, rankColumn, sortOrder = "desc") {
  ...
  data.sort((a, b) => {
    ...
    const scoreB = b[scoreColumn]; // Corrected: was b[scoreB]
...
async function processColumns() {
  ...
  try {
    ...
    const outputHeaders = [
      ...
      "Competition Rank",
      "Population Total",
      "Population Score", // Added right after Population Total
      "Population Rank", // Added right after Population Score
      "Income Average",
      "Traffic Total",
    ];
